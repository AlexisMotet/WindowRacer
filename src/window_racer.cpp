...
int main(int argc, char **argv) {
  ...
  spdlog::info("Num threads: {}", num_entities);


  const double endtime = std::stod(argv[1]);

  mut = std::vector<std::mutex>(num_entities);
  ...
  syn_barrier = new std::barrier<>(num_threads);
  ...
  if (num_threads > 1) {
    for (uint64_t i = 0; i < num_threads; i++) {
      ...
      threads.emplace_back(run_thread, &thread_args[i]);
    }
